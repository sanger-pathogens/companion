...
sub makeOverlap{
  ...
  while (!(-R "tmp.$NAME.$countIT.fasta" && -s "tmp.$NAME.$countIT.fasta")) {
	sleep(3);
 	...
 	sleep(3); 
  ## extract the contigs
  my $call="export PERL5LIB; abacas2.blastOverlaps.pl blast tmp.$NAME.$countIT.fasta tmp.$NAME.$countIT.fasta tmp.$NAME.$countIT.run -report_overlaps -minLength 200 -endTolerance $MAX_CONTIG_END -annotate -annotate_only";
...
# All.overlap.blast.annotated.Overlap 
  ...
  foreach (@F) {
	...
	if (/OVERLAP/) {
	  ...
	  ### here we need the orientation of abacas2 
	  if (($id>=100 && $overlap >= 100)
		  ||
		  (($id>=99 && $overlap >= 1000))
		  ||
		  ## Pacbio setting, large overlaps...
		  (($id>=98 && $overlap >= 2500))
		  ||
		  ## Pacbio setting, large overlaps...
		  (($id>=97 && $overlap >= 5000))
  ...
  if (0 && !defined($contigOverlaps{$contig1}{$contig2})) {
	my %longestOverlap;
