...
tmp=$$
#bsub -J"stage1.$tmp" -R "select[type==X86_64 && mem > 8000] rusage[mem=8000]" -M8000000 -o abacas2.o -e abacas2.e "
~tdo/Bin/abacasII/run.AbacasII.comparison.sh $reference $contig
...
if [ "$doBlast" == "1" ] ; then
	bsub -w "ended(stage3.$tmp)" -J"stage4.$tmp.blast"	~tdo/Bin/abacas2.doblast.sh $reference Res
fi
